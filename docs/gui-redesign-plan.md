...
### 剩余实现顺序
B、C 均已完成。剩余:**G v1.0.1 GPU 统一包** → **A P5 叠加层** → **H 放大器彻底优化** → D/E/M1c。
(顺序可按发布节奏调整;G 紧跟 v1.0.0 首发,H 是独立大块技术债。)
...
**未来规划(不在 v1.0.1)**:① 存放位置统一规划(模型 + CUDA DLL + config 总体布局);② 模型 modal 升级为总 Settings 页面(「模型」入口届时并入)。
...
**✅ 用户测试通过(2026-06-24)**:渲染正确、背景可见、编辑/拖拽/重渲即时精确。**遗留小问题(记录待后续顺手改)**:
1. **编辑框字号轻微不一致**:`.overlay-editor` 用 `region.fontSize`(检测原字号)× scale,而渲染器实际用的是「自适应缩到框内 + clamp 检测±offset」后的字号,故双击编辑时的字号与画布烘焙字号有轻微出入(仅编辑输入框,画布本身精确)。后续可让 `RerenderExport`/`LoadEditable` 回传渲染器**实际用的字号**给前端用。
2. **多行文本只渲染一行(既有渲染器 bug,非 P5 引入)**:部分文本块实际有两行内容,但烘焙结果只显示一行。疑似 png 渲染器 `max_fontsize`/换行/框高自适应逻辑在某些框把第二行挤掉或字号选得过大致溢出裁剪。独立小任务,排查 `crates/modules/renderer/png/src/lib.rs` 的 `render_block`/`max_fontsize`/`find_non_overlapping_position`。
